public static class Trees
{
    ...
    private static void InsertMiddle(int[] sortedNumbers, int first, int last, BinarySearchTree bst)
    {
        ...
        // Base case: when first > last, we're done with this section
        if (first > last)
            return;
        ...
        // Recursively insert the left half (before the middle)
        InsertMiddle(sortedNumbers, first, mid - 1, bst);

        // Recursively insert the right half (after the middle)
